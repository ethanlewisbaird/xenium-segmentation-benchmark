...
def zero_out_diagonal_gpu(sparse_matrix):
    ...
    sparse_matrix_no_diag = scipy_coo_matrix(
        (
...
def subset_sparse_matrix(sparse_matrix, row_idx, col_idx):
    ...
    # Ensure sparse matrix is in COO format for easy indexing
    sparse_matrix = sparse_matrix.tocoo()

    # Create boolean masks for the row and column indices
    row_mask = np.isin(sparse_matrix.row, row_idx)
    col_mask = np.isin(sparse_matrix.col, col_idx)
    ...
    row_mapped = np.searchsorted(row_idx, row_filtered)
    col_mapped = np.searchsorted(col_idx, col_filtered)

    # Return the new subset sparse matrix
    return scipy_coo_matrix(
...
def get_similarity_scores(
    ...
    # Always use kd_tree path (cpu coords) for edge index
    ...
    def sparse_multiply(embeddings, edge_index, shape):
        ...
        columns = indices[1, :].cpu().numpy()
        del indices
        values = similarity[edge_index != -1].flatten().cpu().detach().numpy()
        sparse_result = scipy_coo_matrix(
            (values, (rows, columns)), shape=shape
        )
...
def predict_batch(
    ...
    if len(batch["bd"].pos) >= 10 and len(batch["tx"].pos) >= 1000:
        ...
        # Convert sparse matrix to dense format (NumPy)
        ...
        belongs = np.max(dense_scores, axis=1)  # Max score per transcript
        ...
        assignments["segger_cell_id"] = np.where(
            mask, all_ids[np.argmax(dense_scores, axis=1)], None
        )
        ...
        if use_cc:
            ...
            # Use scipy sparse matrices (CPU)
            no_id_scores = scipy_coo_matrix(
            ...
            no_id = np.where(np.asarray(assignments["segger_cell_id"] == None))[0]
            ...
            if len(no_id) > 0:  # Only compute if there are unassigned transcripts
                ...
                no_id_scores.data[no_id_scores.data < score_cut] = 0
                no_id_scores.eliminate_zeros()  # Clean up zeros

                # Find the non-zero entries in the no_id_scores to construct edge_index
                from scipy.sparse import find as scipy_find
                non_zero_rows, non_zero_cols, _ = scipy_find(no_id_scores)
                unassigned_ids = transcript_id[no_id]  # Unassigned transcript IDs

                # Construct edge index (source, target) based on non-zero connections
                source_nodes = unassigned_ids[non_zero_rows]
                target_nodes = unassigned_ids[non_zero_cols]
