...
class LitSegger(LightningModule):
    ...
    def training_step(self, batch: Any, batch_idx: int) -> torch.Tensor:
        ...
        # Get edge labels and logits — compute only labeled-edge dot products to avoid
        # materializing the full (N_tx × N_bd) matrix which can be multi-GB for large tiles
        edge_label_index = batch["tx", "belongs", "bd"].edge_label_index
        out_values = (z["tx"][edge_label_index[0]] * z["bd"][edge_label_index[1]]).sum(dim=-1)
    ...
    def validation_step(self, batch: Any, batch_idx: int) -> torch.Tensor:
        ...
        # Get edge labels and logits — compute only labeled-edge dot products
        edge_label_index = batch["tx", "belongs", "bd"].edge_label_index
        out_values = (z["tx"][edge_label_index[0]] * z["bd"][edge_label_index[1]]).sum(dim=-1)
        edge_label = batch["tx", "belongs", "bd"].edge_label
